...
public class StartupConfigValidator implements ApplicationListener<ApplicationReadyEvent> {
    ...
    private List<String> validateProductionSecrets() {
        ...
        // Sans secret, les endpoints refusent les événements. Refuser aussi le démarrage
        // en prod pour éviter une intégration Stripe silencieusement indisponible.
        ...
        if (!isBlank(stripeKey) && isBlank(stripeWebhookSecret)) {
            errors.add("STRIPE_WEBHOOK_SECRET is missing while Stripe is configured — webhook signature "
                    + "events would be rejected. Set the webhook signing secret.");
        }
